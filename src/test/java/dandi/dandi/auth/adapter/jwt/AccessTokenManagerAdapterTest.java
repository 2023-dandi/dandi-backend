package dandi.dandi.auth.adapter.jwt;
...
class AccessTokenManagerAdapterTest {
    ...
    private final AccessTokenManagerAdapter accessTokenManagerAdapter = new AccessTokenManagerAdapter(SECRET,
            VALID_DURATION);

    @DisplayName("payload를 받아 token을 생성할 수 있다.")
    @Test
    void generateToken() {
        String token = accessTokenManagerAdapter.generateToken(PAYLOAD);
    ...
    void getPayload() {
        ...
        Long payload = (Long) accessTokenManagerAdapter.getPayload(token);
    ...
    void validate_RiggedToken() {
        assertThatThrownBy(() -> accessTokenManagerAdapter.validate("riggedToken"))
    ...
    void validate_ExpiredToken() {
        ...
        assertThatThrownBy(() -> accessTokenManagerAdapter.validate(expiredToken))
                .isInstanceOf(UnauthorizedException.class)
